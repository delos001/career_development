...
1. Confirm an Interview Completion document exists for the target role. If the file path is not provided, ask for the company name and role title and locate the file using the naming convention `InterviewCompletion_[Company]_[AbbreviatedRole]_[YYYYMM].docx`. If no file is found, inform the user that the Interview Completion document must exist before this skill can proceed — it is generated by `interview_prep`. Do not continue until the file is located.

2. Extract the text content from the Interview Completion document using Python with python-docx. Use the Python executable at `C:/Users/delos/miniconda3/envs/agents/python.exe`. Write a Python script and execute it via Bash. Use Windows-style paths in all file operations. Confirm the extraction succeeded and that the document structure is readable before proceeding.

3. Ask the user which interview round this follow-up is for. Using the extracted content, locate the relevant round's section and read: logistics, interviewer names and titles, questions and responses captured, and the Round Debrief fields. If the specified round's debrief fields are blank, note this explicitly — the user will need to supply that context in Phase 2a.
...
- Paragraph 2: Substantive alignment paragraph. Connect your most relevant experience to the priorities or themes they emphasized. Be specific — cite what they said and what you bring. This is the letter's central value, not a courtesy paragraph.
