...
1. Confirm an Interview Completion document exists for the target role. If the file path is not provided, ask the user for the folder path where application documents for this role are stored, then ask for the company name and abbreviated role title to construct the filename. Search for a file matching `InterviewCompletion_[Company]_[AbbreviatedRole]_[YYYYMM].docx` within that folder. If no file is found, inform the user that the Interview Completion document must exist before this skill can proceed — it is generated by `interview_prep`. Do not continue until the file is located.
...
`Followup_[Company]_[AbbreviatedRole]_Round[N]_[YYYYMM].md`

If individual letters were generated per recipient, append the recipient's last name:
`Followup_[Company]_[AbbreviatedRole]_Round[N]_[LastName]_[YYYYMM].md`
